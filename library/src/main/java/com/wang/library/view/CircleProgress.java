...
public class CircleProgress extends View {
    ...
    public CircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        default_text_size = DpUtil.sp2px( 18);
        min_size = (int) DpUtil.dp2px( 100);
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        ringColor = attributes.getColor(R.styleable.CircleProgress_circle_ring_color, Color.TRANSPARENT);
        ringWidth = attributes.getDimension(R.styleable.CircleProgress_circle_ring_width, (float) DpUtil.dp2px(2));

        ifShowText = attributes.getBoolean(R.styleable.CircleProgress_circle_show_text, false);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        ringRectF.set(ringWidth / 2, ringWidth / 2, MeasureSpec.getSize(widthMeasureSpec) - ringWidth / 2,
                MeasureSpec.getSize(heightMeasureSpec) - ringWidth / 2);
...
//        canvas.drawArc(rectF, 90 - angle, angle * 2, false, paint);
    ...
    private void getCountDownAnimator() {
        if(mCountdownAnimator == null) {
            mCountdownAnimator = ValueAnimator.ofFloat(1, 0);
            mCountdownAnimator.setDuration(5000);
            mCountdownAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    float value = (float)animation.getAnimatedValue();
                    mCountDownValue = value;
                    invalidate();

                }
            });

            mCountdownAnimator.addListener(new Animator.AnimatorListener() {
                @Override
